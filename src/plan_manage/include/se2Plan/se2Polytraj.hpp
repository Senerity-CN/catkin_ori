...
#include <vector>
#include <Eigen/Eigen>
#include <se2Plan/root_finder.hpp>
#define RESDELTA 0.01
namespace se2Plan
{
    ...
    public:
        ...
        PolyTrajectory<1> angleTraj;
        inline double getTotalDuration() const
        {
            return std::min(posTraj.getTotalDuration(), angleTraj.getTotalDuration());
        }
        inline Eigen::Vector2d getPos(double t) const
        {
            return posTraj.getSigma(t);
        }

        inline double getAngle(double t) const
        {
            return angleTraj.getSigma(t)[0];
        }
        inline double getCur(double t) const
        {
            int eta = 1;
            if(getVelNorm(t) < 1.0e-6){
                return 0.0;
            }
            else{
                double yaw = getAngle(t);
                if(get2dVel(t).dot(Eigen::Vector2d(cos(yaw),sin(yaw))) > 0){
                    eta = 1;
                }
                else{
                    eta = -1;
            ...
            return getAngleRate(t) / (eta * sqrt(getVelNorm(t) * getVelNorm(t) + RESDELTA));
        }
        inline double getCur2(double t) const
        {
            int eta = 1;
            if(getVelNorm(t) < 1.0e-6){
                return 0.0;
            ...
            return getAngleRate(t) / (eta * sqrt(getVelNorm(t) * getVelNorm(t) + 10.0 * RESDELTA));
        }
        inline Eigen::Vector2d get2dVel(double t) const
        {
            
            return posTraj.getdSigma(t);
        }
        inline double getVelNorm(double t) const
        {
            return posTraj.getdSigma(t).norm();
        }
        inline double getVel(double t) const
        {
            double yaw = getAngle(t);
            return get2dVel(t).transpose() * Eigen::Vector2d(cos(yaw), sin(yaw));
        }
        inline double getAngleRate(double t) const
        {
            return angleTraj.getdSigma(t)[0];
        }
        inline Eigen::Vector2d get2dAcc(double t) const
        {
            return posTraj.getddSigma(t);
        }
        inline double getLonAcc(double t) const
        {
            Eigen::Vector2d a = get2dAcc(t);
            double yaw = getAngle(t);
            return a[0] * cos(yaw) + a[1] * sin(yaw);
        }
        inline double getLatAcc(double t) const
        {
            Eigen::Vector2d a = get2dAcc(t);
            double yaw = getAngle(t);
            return -a[0] * sin(yaw) + a[1] * cos(yaw);
        }

        inline double getMaxVelRate() const
        {
           
            return posTraj.getMaxDsigmaNorm();
        }

        inline double getMaxAccRate() const
        {
           
            return posTraj.getMaxDdsigmaNorm();
        }
        ...
        inline double getEqfError() const{
            double error = 0.0;
            for(double t = 0.0; t < getTotalDuration(); t+= 0.01){
                Eigen::Vector2d dsigma = get2dVel(t);
                double yaw = getAngle(t);
                error += fabs(dsigma.dot(Eigen::Vector2d(sin(yaw),-cos(yaw))));
            }
            return error;
        }
    };
    ...
    public:
        ...
        inline void initSmGradCost(Eigen::MatrixXd& gdC, Eigen::VectorXd &gdT) {
            gdC.resize(6 * N, Dim); 
            for (int i = 0; i < N; i++)
            {
