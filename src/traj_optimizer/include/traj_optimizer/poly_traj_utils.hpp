...
namespace plan_utils
{
    ...
    public:
        double startTime;
        inline int getSegNum() const { return trajs.size(); }
        inline void clear()
        {
            trajs.clear();
        }
        ...
        inline void push_back(Trajectory tj)
        {
            trajs.push_back(tj);
        }
        ...
        inline Eigen::Vector2d getPos(double t) const
        {
            int trajIdx = locateTrajIdx(t);
            return trajs[trajIdx].getPos(t);
        }
        inline Eigen::Vector2d getdSigma(double t) const
        {
            int trajIdx = locateTrajIdx(t);
            return trajs[trajIdx].getdSigma(t);
        }
        inline Eigen::Vector2d getddSigma(double t) const
        {
            int trajIdx = locateTrajIdx(t);
            return trajs[trajIdx].getddSigma(t);
        }
        inline double getAngle(double t) const
        {
            int trajIdx = locateTrajIdx(t);
            return trajs[trajIdx].getAngle(t);
        }

        inline Eigen::Vector3d getState(double t) const
        {
            int trajIdx = locateTrajIdx(t);
            Eigen::Vector3d state;
            Eigen::Vector2d pos = trajs[trajIdx].getPos(t);
            double yaw = trajs[trajIdx].getAngle(t);
            state << pos, yaw;
            return state;
        }
        ...
        inline int locateTrajIdx(double &t) const
        {
            ...
            for (idx = 0;
                 idx < N &&
                 t > (dur = trajs[idx].getTotalDuration());
                 idx++)
            {
                t -= dur;
            }
