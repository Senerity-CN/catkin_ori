...
private:
  ...
  int traj_res;            // number of distinctive constrain points each piece
...
public:
  ...
  /* set variables */
  void setParam(ros::NodeHandle nh, std::shared_ptr<Config> cfg_);
  ...
  inline int get_traj_resolution_() { return traj_res; };
  ...
  bool OptimizeTrajectory(const std::vector<Eigen::MatrixXd> &iniStates, const std::vector<Eigen::MatrixXd> &finStates,
                          ...
                          std::vector<std::vector<Eigen::MatrixXd>> &hPoly_container, std::vector<int> singuls, double help_eps = 1.0e-4);
...
private:
  ...
  double debugGradCheck(const int i_dp,        // index of constraint point
                        ...
                        const int trajid, const int sur_id, double res_t, Eigen::Matrix<double, 6, 2> c, int i, int j, double omg, double step, double wei_surround_, int K);
