...
// cmd_vel
void rcvCmdvelCallBack(const geometry_msgs::Twist::ConstPtr &msg)
{
	v = msg->linear.x;
	w = msg->angular.z;
}
...
int main(int argc, char **argv)
{
	...
	noise_percent /= 100.0;
	command_sub = nh.subscribe("cmd", 1000, rcvCmdvelCallBack);
	odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 10);
	mesh_pub = nh.advertise<visualization_msgs::Marker>("mesh", 1);
	...
	marker.pose.position.z = 0.0;
	marker.pose.orientation.w = cos(init_yaw / 2.0);
	marker.pose.orientation.x = 0.0;
	marker.pose.orientation.y = 0.0;
	marker.pose.orientation.z = sin(init_yaw / 2.0);
	yaw = init_yaw;
	marker.color.a = 1.0;
	marker.color.r = 0.5;
	marker.color.g = 0.5;
