...
namespace path_searching
{
                        ...
                        Eigen::Vector4d end_state, bool use3d)
  {
    ...
    frontend_map_itf_->CheckIfCollisionUsingPosAndYaw(start_state.head(3), &isocc); // start_state qian3ge yuansu
    if (isocc)
    {
    ...
    while (!open_set_.empty())
    {
      ...
      if ((cur_node->state.head(2) - end_state_.head(2)).norm() < 5.0 && initsearch)
      {
      ...
      Eigen::Vector2d ctrl_input; // steer 控制了机器人的转向.arc 则控制了机器人行进的弧度
      vector<Eigen::Vector2d> inputs;
      double res = 0.5;
      if (!initsearch)
      {
        if (start_state_[3] > 0) // 往前扩展10个节点
        {
      ...
      for (auto &input : inputs) // 范围循环（range-based for loop），它用来遍历容器（如数组、向量、列表）中的元素。
      {
        int singul = input[1] > 0 ? 1 : -1;
        stateTransit(cur_state, pro_state, input); // pro_state 是当前节点cur_state扩展后的节点
        NodeVis(pro_state);
        ...
        tmp_f_score = tmp_g_score + lambda_heu_ * getHeu(pro_state, end_state) + 10 * FindMinDistance(pro_state);
  ...
  bool KinoAstar::is_shot_sucess(Eigen::Vector3d state1, Eigen::Vector3d state2) // 检查两个状态之间的路径是否可行。
  {
  ...
  double KinoAstar::FindMinDistance(const Eigen::Vector3d &center)
  {
    double min_distance = std::numeric_limits<double>::max();
    bool has_obstacle = false; // 初始化为没有障碍物

    for (double x = center[0] - 1.0; x <= center[0] + 1.0; x += 0.25)
    {
      for (double y = center[1] - 1.0; y <= center[1] + 1.0; y += 0.25)
      {
        Eigen::Vector3d test_point(x, y, center[2]);
        bool is_collision; // 新建一个变量用于接收res的值
        frontend_map_itf_->CheckIfCollisionUsingPosAndYaw(test_point, &is_collision);
        if (is_collision)
        { // 根据is_collision的值来判断是否有障碍物
          has_obstacle = true;
          double distance = std::sqrt(std::pow(x - center[0], 2) + std::pow(y - center[1], 2));
  ...
  void KinoAstar::init(ConfigPtr cfg_, ros::NodeHandle nh)
  {
    ...
    check_num_ = cfg_->check_num_; // 5
    max_seach_time = cfg_->max_seach_time;
    step_arc = cfg_->step_arc;
    checkl = cfg_->checkl; // 0.2
    non_siguav = cfg_->non_siguav;
  ...
  {
    sensor_msgs::PointCloud2 globalMap_pcd;  // 定义一个点云消息
    pcl::PointCloud<pcl::PointXYZ> cloudMap; // 定义一个点云对象
    pcl::PointXYZ pt;                        // 定义一个点对象
    // 将状态的 x、y 和固定的 z 值赋给点对象
    pt.x = state[0];
    pt.y = state[1];
    pt.z = 0.2;
    // 将点对象添加到点云对象中
    ...
    pcl::toROSMsg(cloudMap, globalMap_pcd); // 将点云对象转换为 ROS 消息类型
    globalMap_pcd.header.stamp = ros::Time::now();
    globalMap_pcd.header.frame_id = "world";
    expandNodesVis.publish(globalMap_pcd); // 发布扩展节点的可视化信息

    return;
  }
  void KinoAstar::getKinoNode(KinoTrajData &flat_trajs)
  {
    double truncate_len = 1000.0; // 限制或截断生成的轨迹的长度
    bool exceed_len = false;      // not used

    flat_trajs.clear(); // 清除容器中的所有元素
    std::vector<Eigen::Vector3d> roughSampleList;
    double startvel = fabs(start_state_[3]);
    double endvel = fabs(end_state_[3]);
    PathNodePtr node = path_nodes_.back(); // 用path_nodes_容器的最后一个元素来初始化node
    ...
    if (is_shot_succ_)
    {
      ...
      state1 = roughSampleList.back(); // 起点
      state2 = end_state_.head(3);     // 粗糙路径轨迹的最后一点
      from[0] = state1[0];
      from[1] = state1[1];
      from[2] = state1[2];
      to[0] = state2[0];
    ...
    roughSampleList.assign(roughSampleList.begin(), roughSampleList.begin() + truncate_idx + 1); // 将roughSampleList从头开始截取，只保留到索引truncate_idx的元素
    SampleTraj = roughSampleList;
    std::cout << "SampleTraj.size: " << SampleTraj.size() << std::endl;
    /*divide the whole shot traj into different segments*/
    shot_lengthList.clear();
    shot_timeList.clear();
    shotindex.clear();
    shot_SList.clear();
    double tmpl = 0; // temp length
    ...
    std::cout << "shot_lengthList.size: " << shot_lengthList.size() << std::endl;
